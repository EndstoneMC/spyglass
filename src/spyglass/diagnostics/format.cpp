...
std::string hex_dump(const Diagnostic &diagnostic)
{
    ...
    if (diagnostic.raw_truncated) {
        out += std::format("  ... truncated, the body is {} bytes\n", diagnostic.body_size());
    }
    return out;
}
...
std::string to_json(const Diagnostic &diagnostic)
{
    ...
    std::string json = std::format(
        R"({{"sequence":{},"captured_at":"{}","kind":"{}","packet_id":{},"packet_name":"{}")", diagnostic.sequence,
        diagnostic.captured_at, diagnostic.failure == Failure::DecodeError ? "decode_error" : "trailing_bytes",
...
{
    const auto &stream = diagnostic.stream;
    auto report = std::format("{}  {} ({} / {:#x})\n", diagnostic.captured_at, diagnostic.packet_name,
                              diagnostic.packet_id, static_cast<unsigned>(diagnostic.packet_id));
    ...
    report += std::format("Cursor {}/{}, body starts at {}, {} unread{}\n\n", stream.cursor, stream.length,
                          stream.body_begin, stream.unread, stream.overflowed ? ", stream overflowed" : "");

    if (!diagnostic.frames.empty()) {
        report += "Bedrock call stack (innermost first):\n";
        for (const auto &frame : diagnostic.frames) {
            report +=
                std::format("  {}{}:{}", std::string(frame.depth * 2, ' '), file_name(frame.filename), frame.line);
            if (!frame.context.empty()) {
