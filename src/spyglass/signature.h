...
// Cut against Minecraft.Windows.exe 1.26.40.5 and the x86_64 libminecraftpe.so of the 1.26.44.3
// Android release. Neither build names these functions at runtime. The Windows client carries RTTI
// for webrtc only, and the Android .dynsym drops every game symbol, so both are located the same
// way: from the strings that survive the strip, and from the exception-table function index that
// turns an address back into the function holding it. That index is .pdata on Windows and
// .eh_frame_hdr on Android. Follow the recipes again after a client update. Do not widen a pattern
// to make it match.
//
...
//    bytes on 1.26.40.5 and 0x2080 on 1.26.44.3, and it takes each packet off the peer, creates it
//    and reads it
...
//    with the protocol. it is 0x15f on 1.26.40.5 and 1.26.44.3 and 0x161 on the 1.26.50.27
//    preview, so match the shape and read the constant off the match
// 5. readNoHeader is the callee in that same lambda that nothing else in the binary calls. it is
//    0x204 bytes on Windows and 0x270 on Android, and it opens by storing the SubClientId byte it
//    dereferences into the packet, then calls Packet::_read through vtable slot 0x48
...
constexpr std::string_view kBatchedSendPacket =
    ...
    "00 00 48 89 45 D0 48 83 C7 18 44 0F B6 3E 41 F6 C7 01 74 06 4D 8B 66 08 EB 06 45 89 FC 41 D1 EC";
constexpr std::string_view kPacketReadNoHeader =
    "55 48 89 E5 41 57 41 56 41 54 53 48 81 EC E0 00 00 00 48 89 FB 64 48 8B 04 25 28 00 00 00 48 89 45 "
    "D8 41 0F B6 00 88 46 10 48 8B 06 48 8D BD 48 FF FF FF FF 50 48 0F B6 45 88 88 45 D0 84 C0 74 07";
constexpr std::string_view kCreatePacket =
    "55 48 89 E5 53 50 48 89 FB 81 FE 5F 01 00 00 77 12 89 F0 48 8D 0D ? ? ? ? 48 63 04 81 48 01 C8 FF "
    "E0 0F 57 C0 0F 11 03";
#endif
